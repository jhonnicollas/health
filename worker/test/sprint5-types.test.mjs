...
const workerTypesUrl = new URL('worker/src/shared-types/sprint5.ts', root)
const webTypesUrl = new URL('web/src/types/sprint5.ts', root)
...
test('Sprint 5 type constants mirror schema and seed source of truth', async () => {
  ...
  assert.deepEqual(unique(constArray(workerTypes, 'SPRINT5_TABLES')), schemaTables)

  const roleCodes = unique([...insertBlock(seed, 'HL_roles').matchAll(/\('([^']+)',/g)].map((item) => item[1]))
  assert.deepEqual(unique(constArray(workerTypes, 'SPRINT5_ROLE_CODES')), roleCodes)

  const planCodes = unique([...insertBlock(seed, 'HL_plans').matchAll(/\('([^']+)',/g)].map((item) => item[1]))
  assert.deepEqual(unique(constArray(workerTypes, 'SPRINT5_PLAN_CODES')), planCodes)

  const permissionCodes = unique([...seed.matchAll(/\('((?:admin|feature|family)\.[^']+)',/g)].map((item) => item[1]))
  assert.deepEqual(unique(constArray(workerTypes, 'SPRINT5_PERMISSION_CODES')), permissionCodes)

  const featureCodes = unique(permissionCodes.filter((code) => code.startsWith('feature.')))
  assert.deepEqual(unique(constArray(workerTypes, 'SPRINT5_FEATURE_CODES')), featureCodes)
  ...
  assert.deepEqual(unique(constArray(workerTypes, 'SPRINT5_CONFIG_KEYS')), configKeys)
})

test('Sprint 5 guards narrow only known seed values', async () => {
  const workerTypes = await read(workerTypesUrl)

  assert.ok(constArray(workerTypes, 'SPRINT5_ROLE_CODES').includes('superAdmin'))
  assert.ok(constArray(workerTypes, 'SPRINT5_PERMISSION_CODES').includes('admin.aiClinicalCopilot.manage'))
  assert.ok(constArray(workerTypes, 'SPRINT5_FEATURE_CODES').includes('feature.vectorMemory.use'))
