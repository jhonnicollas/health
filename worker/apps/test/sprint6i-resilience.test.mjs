...
test('S6I-T-09 AI_SERVICE timeout (throw) → 502', async () => {
  const env = {
    DB: createDbMock(),
...
test('S6I-T-09 AI_SERVICE unavailable (undefined binding) → 503', async () => {
  const env = {
    DB: createDbMock(),
  };
  const res = await callProbe(env);
...
test('S6I-T-09 100 concurrent resilience calls → no crashes and expected codes', async () => {
  ...
  assert.ok(codes.every((c) => [200, 502, 503].includes(c)), `Unexpected status codes: ${codes.filter((c) => ![200, 502, 503].includes(c)).join(',')}`);
